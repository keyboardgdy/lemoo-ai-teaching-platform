...
| Git 仓库          | W2 已初始化      | `main` 保存治理基线；工程变更位于 `prep/w2-repo-toolchain` |
| 远程仓库            | W2 已创建，保护待解锁 | 私有 `keyboardgdy/lemoo-ai-teaching-platform`；原有公开 `keyboardgdy/Lemoo` 为不相容 C# 项目，不覆盖；当前 GitHub 套餐拒绝私有仓库分支保护 |
...
**执行状态（2026-08-13）**：实现、Windows 验证及 PR #1 的 Linux `governance/backend/frontend/compose/security` 五项检查已通过，证据见 [`docs/gates/evidence/W2/`](gates/evidence/W2/README.md)。远端使用私有 `keyboardgdy/lemoo-ai-teaching-platform`，因为账户内同名大小写不敏感地址已被不相容的公开 C# 项目占用。GitHub API 明确返回当前套餐需升级 GitHub Pro 或公开仓库才能保护私有 `main`；项目保持私有，W2 状态为 `blocked_branch_protection`，在升级套餐或高端阳书面接受限时例外前不进入依赖 W2 的工作包。
