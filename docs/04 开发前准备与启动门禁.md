...
| 远程仓库            | W2 已创建       | 私有 `keyboardgdy/lemoo-ai-teaching-platform`；原有公开 `keyboardgdy/Lemoo` 为不相容 C# 项目，不覆盖 |
| 工程代码            | W2 空业务骨架已建立 | FastAPI/Vue 只提供系统状态与健康检查；领域能力保持禁用 |
| CI/CD           | W2 最小 CI 已建立 | Windows 本地通过；Linux 结果以远端 PR Check 为准 |
...
**执行状态（2026-08-13）**：Windows 本地实现与验证已完成，证据见 [`docs/gates/evidence/W2/`](gates/evidence/W2/README.md)。远端使用私有 `keyboardgdy/lemoo-ai-teaching-platform`，因为账户内同名大小写不敏感地址已被不相容的公开 C# 项目占用；远端 Linux CI 与分支保护结果以 W2 PR 和证据记录为准。
