...
| Git 仓库          | 未初始化         | 必须完成                                    |
| 远程仓库            | 未配置          | 必须确定平台、权限和保护规则                          |
| 工程代码            | 不存在          | 准备阶段创建最小骨架                              |
| CI/CD           | 不存在          | 准备阶段创建最小门禁                              |
| 本地依赖环境          | 不存在          | 准备阶段建立 Compose                          |
| 真实设备证据          | 当前无物理设备      | 不阻塞阶段 1A Simulator 开发；硬阻塞真实设备/机构试点和生产声明 |
| 协议 Schema       | 不存在          | 设备功能开发前硬阻塞                              |
| Robot Simulator | 不存在          | 设备云 MVP 开发前硬阻塞                          |
...
| Git            | 2.51.0              | 需要              | 可用                                 |
| GitHub CLI     | 2.93.0              | 推荐              | 可用；仍需检查登录和仓库权限                     |
| Python         | 3.12.10             | Python 3.14     | 不一致；通过 uv 安装并锁定 3.14               |
| uv             | 0.12.2              | 需要              | 可用                                 |
| Node.js        | 24.11.1             | Node 24 LTS     | 可用                                 |
| pnpm           | 11.2.2              | 需要              | 可用；通过 `packageManager` 锁定          |
| Docker CLI     | 29.1.3              | 需要              | 已安装                                |
| Docker Compose | 5.0.1               | 需要              | 已安装                                |
| Docker Engine  | 当前未运行               | 需要              | 启动 Docker Desktop/Linux Engine 后验证 |
| Task           | 未安装                 | Taskfile 基线     | 安装或通过 ADR 删除该依赖                    |
