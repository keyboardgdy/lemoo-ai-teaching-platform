...
def test_generated_pki_has_separate_ca_server_and_smoke_client_identities(tmp_path) -> None:
    ...
    assert x509.ExtendedKeyUsageOID.SERVER_AUTH in server_usage
    assert x509.ExtendedKeyUsageOID.CLIENT_AUTH in client_usage
...
def test_generation_is_idempotent_while_the_complete_bundle_is_valid(tmp_path) -> None:
    now = datetime(2026, 8, 14, 3, 0, tzinfo=UTC)
